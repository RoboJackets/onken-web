...
Router.onRouteChangeStart = url => {
  if (window.location.pathname !== url) NProgress.start();
};
...
class Layout extends Component {
  ...
  onWindowResize = () => {
    const isNarrow = window.matchMedia(
      `(max-width: ${this.props.theme.sidenavWidth * 3}em)`
    ).matches;
  ...
  shouldComponentUpdate = (nextProps, nextState) => {
    return (
      this.state.showNav !== nextState.showNav ||
  ...
  render() {
    return (
      this.state.hasOwnProperty("showNav") && (
        <Container>
          <UserProvider>
